...
export default {
  ...
  computed: {
    hasInstitutions() {
      if (!Array.isArray(this.repository?.institutions)) {
        return false;
      }
      return this.repository.institutions.length > 0;
    },
    sortedInstitutions() {
      if (!Array.isArray(this.repository?.institutions)) { return []; }

      return this.repository.institutions.slice().sort(
  ...
  methods: {
    ...
    onChange() {
      this.$emit('updated', this.repository?.institutions ?? []);
    },
    ...
    async saveInstitution() {
      ...
      try {
        ...
        if (!this.repository.institutions.some((i) => i?.pattern === this.selectedInstitution.id)) {
          this.repository = {
            ...this.repository,
            institutions: [...this.repository.institutions, this.selectedInstitution],
          };
        }
    ...
    async removeInstitution(institutionId) {
      ...
      try {
        ...
        this.repository = {
          ...this.repository,
          institutions: this.repository.institutions.filter((i) => i?.id !== institutionId),
        };
        this.onChange();
      } catch (e) {
