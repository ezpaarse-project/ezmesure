...
export default {
  ...
  methods: {
    ...
    addSubInstitution(subInstitution) {
      this.hasChanged = true;
      this.subInstitutions.push(subInstitution);
    },
    ...
    async refreshSubInstitutions() {
      ...
      try {
        this.subInstitutions = await this.$axios.$get(
          `/institutions/${this.institution.id}/subinstitutions`,
    ...
    async saveSubInstitution() {
      ...
      try {
        await this.$axios.$put(`/institutions/${this.institution.id}/subinstitutions/${selectedInstitition.id}`);
    ...
    async removeSubInstitution(institutionId) {
      ...
      try {
        await this.$axios.$delete(`/institutions/${this.institution.id}/subinstitutions/${institutionId}`);
